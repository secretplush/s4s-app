...
// Upload a single image to a model's OF account, return the mediaId
// Body: { targetUsername, base64, filename }
// Response: { mediaId } or { error }
export async function POST(req: NextRequest) {
  try {
    const { targetUsername, base64, filename } = await req.json()

    if (!targetUsername || !base64) {
      return NextResponse.json({ error: 'Missing targetUsername or base64' }, { status: 400 })
    }
