#!/bin/bash
# Cross-check 71 ad fans across all Plush accounts
...
echo "Found $TOTAL_ACCTS authenticated accounts, ${#FANS[@]} fans to check"
...
for acct_line in $ACCOUNTS; do
    ...
    for fan_id in "${FANS[@]}"; do
        DATA=$(curl -s -H "$AUTH" "$BASE/$ACCT_ID/users/$fan_id")
        
        # Check if fan is subscribed
        SUB_BY=$(echo "$DATA" | python3 -c "
import json,sys
try:
    d=json.load(sys.stdin)
    u=d.get('data',d)
    if u.get('subscribedBy'):
        so=u.get('subscribedOnData',{})
        print(json.dumps({
            'fan_id':$fan_id,
            'account_id':'$ACCT_ID',
            'model':'$MODEL',
...
" 2>/dev/null)
        ...
        if [ -n "$SUB_BY" ]; then
            echo "$SUB_BY" >> "$RESULTS"
            FOUND=$((FOUND + 1))
            T=$(echo "$SUB_BY" | python3 -c "import json,sys;print(json.load(sys.stdin).get('totalSumm',0))")
            if [ "$T" != "0" ]; then
                SPENT=$((SPENT + 1))
            fi
        fi
    ...
    echo "$FOUND fans found, $SPENT spent money"
done
